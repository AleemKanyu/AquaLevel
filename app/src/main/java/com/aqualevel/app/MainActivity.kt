...
class MainActivity : AppCompatActivity() {
    ...
    private var availableDevices: List<Device> = emptyList()

    override fun onCreate(savedInstanceState: Bundle?) {
        sharedPref = getSharedPreferences("AquaLevelPrefs", Context.MODE_PRIVATE)

        // Sign in anonymously so Firestore rules work — user never sees a login screen
        val auth = com.google.firebase.auth.FirebaseAuth.getInstance()
        if (auth.currentUser == null) {
            auth.signInAnonymously()
        }

        // If no device paired yet, send to pairing screen
        ...
        val addDeviceBtn = findViewById<FrameLayout>(R.id.addDeviceBtn)
        ...
        checkForUpdates()
        maybeRequestPlayStoreRating()
    }

    override fun onResume() {
        super.onResume()
        if (::viewModel.isInitialized) {
            // PairDeviceActivity updates SharedPreferences; refresh the selected device
            // when MainActivity comes back so newly added ESP32s start syncing immediately.
    ...
    private fun maybeRequestPlayStoreRating() {
        if (sharedPref.getString("user_name", null) == null) return

        val now = System.currentTimeMillis()
        val launchCount = sharedPref.getInt(KEY_RATE_LAUNCH_COUNT, 0) + 1
        val lastRequestAt = sharedPref.getLong(KEY_RATE_LAST_REQUEST_AT, 0L)
        val requestCount = sharedPref.getInt(KEY_RATE_REQUEST_COUNT, 0)

        sharedPref.edit()
            .putInt(KEY_RATE_LAUNCH_COUNT, launchCount)
            .apply()

        if (launchCount < RATE_MIN_LAUNCHES) return
    ...
    private fun scheduleBackgroundWorker() {
        val workRequest = PeriodicWorkRequestBuilder<WaterLevelWorker>(1, TimeUnit.HOURS).build()
    ...
    companion object {
        private const val KEY_RATE_LAUNCH_COUNT = "play_store_rating_launch_count"
        ...
        private const val RATE_PROMPT_DELAY_MS = 4_000L
